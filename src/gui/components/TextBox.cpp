...
void TextBox::draw(Graphics &graphics) {
    ...
    if (is_focused()) {
        glm::vec2 textBounds = GameWindow::get_instance()->get_font_renderer()->get_bounds(text, fontSize);
        glm::vec2 textUntil = GameWindow::get_instance()->get_font_renderer()->get_bounds(text.substr(0, cursorPos),
                                                                                          fontSize);
